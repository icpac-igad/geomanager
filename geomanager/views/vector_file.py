...
def upload_vector_file(request, dataset_id=None, layer_id=None):
    ...
    file_error_messages = {
        "invalid_file_extension": _("Not a supported vector format. Supported formats: %(supported_formats)s.")
    ...
    if request.method == "POST":
        ...
        initial_data = {"layer": layer_id if layer_id else query_set.first(), "table_name": filename_without_ext}
...
def publish_vector(request, upload_id):
    ...
    if layer_form.is_valid():
        ...
        data = {"layer": layer, "time": time, "table_name": table_name, "description": description}
        ...
        data.update(
            {"full_table_name": full_table_name, "properties": properties, "bounds": bounds, "geometry_type": geom_type}
        )
...
def preview_vector_layers(request, dataset_id, layer_id=None):
    ...
    vector_layer_list_url = vector_layer_list_url + f"?dataset__id__exact={str(dataset_id)}"

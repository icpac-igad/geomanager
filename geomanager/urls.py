...
from .views import (
    ...
    GeoJSONPgTableView,
    AdditionalBoundaryVectorTileView,
    get_boundary_data_feature_by_id,
)
from .views.auth import EmailTokenObtainPairView, UserTokenVerifyView
...
urlpatterns = [
    ...
    path(
        r"mapviewer/<str:location_type>/<str:adm0>/<str:adm1>/",
        map_view,
        name="mapview",
    ),
    path(
    ...
    path(
        r"mapviewer/<str:location_type>/<str:adm0>/<str:adm1>/<str:adm2>/",
        map_view,
        name="mapview",
    ),
    ...
    # User Profile
    path(
        "api/geomanager-profile/<int:user_id>",
        get_geomanager_user_profile,
        name="get_geomanager_user_profile",
    ),
    path(
        "api/geomanager-profile/update/<str:user_id>",
        create_or_update_geomanager_user_profile,
    ...
    path(
        r"api/country",
        AdminBoundaryViewSet.as_view({"get": "get"}),
        name="country_list",
    ),
    path(
        r"api/country/<str:gid_0>",
        AdminBoundaryViewSet.as_view({"get": "get_regions"}),
        name="country_regions",
    ),
    path(
        r"api/country/<str:gid_0>/<str:gid_1>",
    ...
    path(
        r"api/geostore/<uuid:geostore_id>",
        GeostoreViewSet.as_view({"get": "get"}),
        name="get_by_geostore",
    ),
    path(
        r"api/geostore/admin/<str:gid_0>",
        GeostoreViewSet.as_view({"get": "get_by_admin"}),
        name="get_by_gid0",
    ),
    path(
        r"api/geostore/admin/<str:gid_0>/<str:gid_1>",
        GeostoreViewSet.as_view({"get": "get_by_admin"}),
        name="get_by_gid1",
    ),
    path(
        r"api/geostore/admin/<str:gid_0>/<str:gid_1>/<str:gid_2>",
        GeostoreViewSet.as_view({"get": "get_by_admin"}),
        name="get_by_gid2",
    ),
    # Boundary feature by id
    path(
        r"api/map-boundary-data/<str:table_name>/<int:gid>",
        get_boundary_data_feature_by_id,
        name="boundary_data_feature_by_id",
    ),
    # Tile JSON
    path(
    ...
    # Tiles
    path(
        r"api/raster-tiles/<uuid:layer_id>/<int:z>/<int:x>/<int:y>",
        RasterTileView.as_view(),
        name="raster_tiles",
    ),
    path(
        r"api/vector-tiles/<int:z>/<int:x>/<int:y>",
        VectorTileView.as_view(),
        name="vector_tiles",
    ),
    path(
        r"api/map-boundary-tiles/<str:table_name>/<int:z>/<int:x>/<int:y>",
        AdditionalBoundaryVectorTileView.as_view(),
        name="map_boundary_tiles",
    ),
    # Thumbnail
    path(
        r"api/raster-file-thumbnail/<int:file_id>.png",
        RasterThumbnailView.as_view(),
        name="raster_file_thumbnail",
    ),
    # Data
    path(
        r"api/raster-data/pixel/<uuid:layer_id>",
        RasterDataPixelView.as_view(),
        name="raster_data_pixel",
    ),
    path(
        r"api/raster-data/pixel/timeseries/<uuid:layer_id>",
        RasterDataPixelTimeseriesView.as_view(),
        name="raster_data_pixel_timeseries",
    ),
    path(
        r"api/raster-data/geostore/<uuid:layer_id>",
        RasterDataGeostoreView.as_view(),
        name="raster_data_geostore",
    ),
    path(
        r"api/raster-data/geostore/timeseries/<uuid:layer_id>",
        RasterDataGeostoreTimeseriesView.as_view(),
        name="raster_data_geostore_timeseries",
    ),
    # FeatureServ
    path(
        r"api/feature-serv/<str:table_name>.geojson",
        GeoJSONPgTableView.as_view(),
        name="feature_serv",
    ),
    # Tiles GL
    path(
        r"api/tile-gl/tile/<str:source_slug>/<int:z>/<int:x>/<int:y>.pbf",
        tile_gl,
        name="tile_gl",
    ),
    path(
        r"api/tile-gl/tile-json/<str:source_slug>.json",
        tile_json_gl,
        name="tile_json_gl",
    ),

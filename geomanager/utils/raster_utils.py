    ...
    try:
        ...
        if value_type:
            if value_type == "mean":
                ...
                data.update({"min": values.min(), "max": values.max()})
            elif value_type == "minmeanmax":
                data.update({"min": values.min(), "max": values.max(), "mean": values.mean()})
            else:
                data.update({"mean": values.mean()})
        else:
            data.update({"mean": values.mean()})
...
def read_raster_info(file_path):
    ...
    if raster_info["driver"] == "netCDF":
        ...
        if time_dim_name:
            ...
            raster_info.update({"time_dimension_name": time_dim_name, "timestamps": timestamps})
...
def convert_upload_to_geotiff(upload, out_file_path, band_index=None, data_variable=None):
    ...
    if driver == "GTiff":
        ...
        # save as COG
        cog_translate(upload.file.path, out_file_path, output_profile, indexes=band_index, in_memory=False)

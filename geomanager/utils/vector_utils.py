...
def ogr2pg(file_path, table_name, db_settings, srid=4326, overwrite=False):
    ...
    cmd = [
        ...
        "-f",
        "PostgreSQL",
        f"PG:host={db_host} port={db_port} user={db_user} password={db_password} dbname={db_name}",
        file_path,
        "-nln",
        table_name,
        "-lco",
...
def create_feature_collection_from_geom(geom):
    return {
        ...
        "features": [{"type": "Feature", "geometry": json.loads(geom.geojson), "properties": {}}],
    }

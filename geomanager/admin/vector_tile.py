...
from geomanager.models import Dataset, VectorTileLayer, Category
from geomanager.views.vector_tile import preview_vector_tile_layers
...
class VectorTileLayerCreateView(CreateView):
    ...
    def get_context_data(self, **kwargs):
        context_data = super(VectorTileLayerCreateView, self).get_context_data(**kwargs)
...
class VectorTileLayerEditView(EditView):
    def get_context_data(self, **kwargs):
        context_data = super(VectorTileLayerEditView, self).get_context_data(**kwargs)
        ...
        layer_admin_helper = AdminURLHelper(VectorTileLayer)
        layer_index_url = layer_admin_helper.get_action_url("index")

        navigation_items = [
            {"url": category_index_url, "label": Category._meta.verbose_name_plural},
            {"url": datasets_index_url, "label": Dataset._meta.verbose_name_plural},
            {"url": layer_index_url, "label": VectorTileLayer._meta.verbose_name_plural},
...
class VectorTileLayerModelAdmin(BaseModelAdmin, ModelAdminCanHide):
